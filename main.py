...
def camera_thread_function():
    ...
    try:
        ...
        # Initialize Firebase Storage bucket if Firebase is enabled
        bucket = None
        if firebase_enabled:
            try:
                # Get the default bucket from Firebase
                bucket = storage.bucket()
                print("Firebase Storage bucket initialized")
            except Exception as e:
        ...
        while camera_active:
            ...
            if next_interval_time is None or current_time >= next_interval_time:
                ...
                if "error" not in results and firebase_enabled:
                    ...
                    image_url = None
                    if bucket is not None and os.path.exists(output_path):
                        try:
                            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                            image_path = f"solar_panel_images/{timestamp}.jpg"
                            
                            # Upload image to Firebase Storage
                            blob = bucket.blob(image_path)
                            blob.upload_from_filename(output_path)
                            
                            # Make the image publicly accessible (optional, based on security needs)
                            blob.make_public()
                    ...
                    # Log model status with image URL
                    log_data = {
                        "model_details": {
                        ...
                        "image_url": image_url,  # Add image URL to the log
                        "timestamp": datetime.now().isoformat()
                    }
